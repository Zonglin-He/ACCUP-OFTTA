...
def _gen_priors(num_bn, alpha0=0.7, lam=0.7):
    # alpha_i = 1 - alpha0 * exp(-lam * i)   （i 从 0 起）
...
def patch_bn_to_edtn(model: nn.Module, alpha0=0.7, lam=0.7, verbose=True):
    ...
    if verbose:
        print(f"[EDTN] Replaced {replaced} BN layers. "
              f"alpha0={alpha0:.3f}, lambda={lam:.3f}, "
              f"prior[0](CBN)={priors[0]:.3f} / TBN={1-priors[0]:.3f}, "
              f"prior[-1](CBN)={priors[-1]:.3f} / TBN={1-priors[-1]:.3f}")
    return model
